...
const fetchdata = async () => {
  const response = await fetch(
    "https://newsapi.org/v2/top-headlines?country=in&apiKey=e15a87d942a840fca586073cfc8894c6&pageSize=10&page=1&category=entertainment"
  );
  ...
  json.articles.map((ele) => {
    ...
    y.setAttribute(
      "src",
      `${ele.urlToImage ? ele.urlToImage : "/breaking.jpg"}`
    );
    y.setAttribute("alt", "");
...
window.addEventListener("scroll", () => {
  ...
  if (
    ...
    setTimeout(() => {
      fetch(
        `https://newsapi.org/v2/top-headlines?country=in&apiKey=e15a87d942a840fca586073cfc8894c6&pageSize=10&page=${page}&category=entertainment`
      )
        .then((res) => res.json())
        .then((json) => {
          json.articles.map((ele) => {
            ...
            publish.innerHTML = `${ele.publishedAt}`;
            content.innerHTML = `${
              ele.content ? ele.content.slice(0, 40) : ""
            }`;
            z.append(auther);
            z.append(publish);
            z.append(content);
